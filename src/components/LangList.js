...
function getLangProgress(lang, issue) {
  const { corePages = "Core Pages", nextSteps = "Next Steps", ...langProps } = lang
...
async function getProgressList(langs) {
  ...
  const { search } = await graphql(
    `
      query($limit: Int!) {
        search(type: ISSUE, query: "org:gatsbyjs Translation Progress in:title", first: $limit) {
          nodes {
            ... on Issue {
              title
              body
  ...
  const issuesMap = fromPairs(
    search.nodes.filter(issue => !!issue && issue.repository).map(issue => [issue.repository.name, issue])
  )
...
export default function LangList({ langs }) {
  ...
  const sortedList = useMemo(() => {
    ...
    if ((Array.isArray(sortKey) && sortKey.includes("coreCompletion")) || sortKey === "lastEditedAt") {
      sorted.reverse()
    }
    return sorted
  ...
  return (
    <div>
      <SortSelector options={sortOptions} value={sortKey} onSelect={setSortKey} />
      <div {...style}>
        {sortedList.map(lang => (
